...
const NurseManagement = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Filter nurses by current clinic admin's clinic
      if (user?.role === "clinic_admin" && user?.clinicId) {
        nursesData = (nursesData || []).filter((nurse) => {
          const nurseClinicId =
            nurse.clinicId?._id || nurse.clinicId || nurse.clinic?._id;
          return nurseClinicId === user.clinicId;
        });
      }
      ...
      console.log("Filtered nurses count:", nursesData?.length);
  ...
  const fetchClinics = async () => {
    try {
      ...
      // Filter clinics for clinic admin - only show their clinic
      if (user?.role === "clinic_admin" && user?.clinicId) {
        clinicsData = (clinicsData || []).filter(
          (clinic) => clinic._id === user.clinicId
        );
      }
  ...
  const NurseModal = ({ nurse, isOpen, onClose }) => {
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white dark:bg-gray-950 rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
          <div className="p-6">
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Nurses</h1>
          <p className="text-gray-600 dark:text-gray-400">
            {user?.role === "clinic_admin"
              ? `Manage nursing staff for ${clinics[0]?.name || "your clinic"}`
      ...
      <div className="card dark:bg-gray-950 dark:border-gray-800">
        <div className="flex flex-col lg:flex-row gap-4 mb-6">
          ...
          {/* Clinic Filter - Hide for clinic admin since they only see their clinic */}
          {user?.role !== "clinic_admin" && (
          ...
          {/* Filter Status */}
          {(searchTerm ||
            (user?.role !== "clinic_admin" && filterClinic !== "all") ||
            filterStatus !== "all") && (
