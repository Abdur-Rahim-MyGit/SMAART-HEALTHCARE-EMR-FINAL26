...
const { pingMongo, supportsTransactions } = require('../../infrastructure/mongodb/connection');
...
async function readiness() {
  ...
  checks[0].transactions = supportsTransactions();
  if (redis.isMemory) checks[1].mode = 'memory-fallback';
...
router.get('/', asyncHandler(async (_req, res) => {
  ...
  res.status(r.ready ? 200 : 503).json({ success: r.ready, status: r.ready ? 'ok' : 'degraded', service: config().APP_NAME, version: process.env.npm_package_version || '3.0.0', env: config().NODE_ENV, startedAt, uptimeSeconds: Math.round(process.uptime()), checks: r.checks });
}));
