'use strict';
/**
 * Pure mapping from legacy Mongo documents to the new MongoDB model. Rows are
 * produced with snake_case keys and turned into camelCase documents by run.js.
 */
...
const json = (v, def) => (v === undefined || v === null ? def : v);
const GENDER = { male: 'male', m: 'male', female: 'female', f: 'female', other: 'other' };
...
async function buildPlan(src, { uuidFor, report, hashPassword, ROLES }) {
  ...
  const mongo = { clinicalNotes: [], communityPosts: [], activityLogs: [] }; // flexible documents (clinical_notes, community_posts, patient_activity_logs)
  ...
  for (const p of src.patients) {
    ...
    tables.patients.push({ __legacy: legacy('patients', p._id), id, clinic_id: clinic, full_name: p.fullName || 'Unknown', date_of_birth: date(p.dateOfBirth), gender: GENDER[String(p.gender || '').toLowerCase()] || 'unknown', phone: str(p.phone || p.attenderMobile), email: lower(p.email), blood_group: str(p.bloodGroup || p.bloodType), marital_status: typeof p.maritalStatus === 'boolean' ? (p.maritalStatus ? 'Married' : 'Single') : str(p.maritalStatus), nationality: str(p.nationality), occupation: str(p.occupation), mode_of_care: str(p.modeOfCare), city: str(p.city || p.address?.city), pin_code: str(p.pinCode || p.address?.zipCode), address: json(typeof p.address === 'object' ? p.address : p.address ? { street: p.address } : {}, {}), emergency_contact: json(p.emergencyContact, {}), insurance: json(p.insurance || p.insuranceInfo, {}), attender_email: lower(p.attenderEmail), attender_mobile: str(p.attenderMobile), attender_whatsapp: str(p.attenderWhatsapp), referring_doctor: str(p.referringDoctor), referred_clinic: str(p.referredClinic), hand_dominance: str(p.handDominance), is_under_18: !!p.isUnder18, parent_guardian: json(p.parentGuardian, {}), notes: str(p.notes), profile_image_url: (p.profileImage || p.imageUrl) && /^https?:/.test(p.profileImage || p.imageUrl) ? p.profileImage || p.imageUrl : null, status: 'active', last_visit_at: date(p.lastVisit), next_appointment_at: date(p.nextAppointment), attributes: attrs, created_at: date(p.createdAt) || new Date() });
  ...
  for (const c of src.consultations) {
    ...
    for (const lt of c.labTests || []) tables.lab_orders.push({ id: uuidFor('consultation-lab', `${c._id}:${lt._id || lt.testName}`), clinic_id: clinic, patient_id: patient, encounter_id: id, test_name: lt.testName || 'Lab test', priority: ['Routine', 'Urgent', 'Emergency'].includes(lt.priority) ? lt.priority : 'Routine', test_code: null, status: 'completed', ordered_at: date(c.createdAt) || new Date(), notes: str(lt.notes), created_at: date(c.createdAt) || new Date() });
  }
  ...
  for (const v of src.vitals) {
    ...
    tables.vitals.push({ __legacy: legacy('vitals', v._id), id: uuidFor('vitals', v._id), clinic_id: clinic, patient_id: patient, recorded_at: date(v.visitDate) || date(v.createdAt) || new Date(), recorded_by_name: str(v.recordedByName), recorded_by_role: str(v.recordedByRole), systolic: num(vs.bloodPressure?.systolic), diastolic: num(vs.bloodPressure?.diastolic), heart_rate: val(vs.heartRate), temperature: val(vs.temperature), temperature_unit: vs.temperature?.unit || '°F', respiratory_rate: val(vs.respiratoryRate), oxygen_saturation: (() => { const o = val(vs.oxygenSaturation); return o !== null && o >= 0 && o <= 100 ? o : null; })(), weight_kg: val(vs.weight), height_cm: val(vs.height), bmi: val(vs.bmi), bmi_category: str(vs.bmi?.category), blood_sugar: val(vs.bloodSugar), notes: str(v.notes), clinical_notes: json(v.clinicalNotes, {}), created_at: date(v.createdAt) || new Date() });
  }
